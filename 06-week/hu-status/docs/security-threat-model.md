...
| R-3 | It is impossible to determine which user registered a specific sale | sales-service | **Current gap.** The `sales` table has no `created_by` field. This gap will be resolved by ADR-002, which adds sale authorship traceability | Pending (ADR-002) |
...
| I-4 | A user with the SALESPERSON role accesses another salesperson's sales data | sales-service | **Current gap.** Without the `created_by` field in the `sales` table, it is impossible to filter sales by user. Will be resolved by ADR-002 | Pending (ADR-002) |
...
| Spoofing | 3 | 3 | 0 | 0 |
| Tampering | 3 | 3 | 0 | 0 |
| Repudiation | 3 | 2 | 1 (ADR-002) | 0 |
| Information Disclosure | 4 | 3 | 1 (ADR-002) | 0 |
...
* Sale authorship decision → `05-architecture/decisions/records/ADR-002-*.md` (pending)
