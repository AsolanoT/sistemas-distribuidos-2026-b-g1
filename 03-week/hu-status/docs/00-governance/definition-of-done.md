...
The following exceptions must be explicitly agreed to by the team:
...
- "The instructor approved it in class" — that is the product Definition of Done, not the code's

---

## Note on Sprint 0 (discovery documentation)

For `TASK` and `SPIKE` items in weeks 1-3 (PDR, ADR, context map), the applicable Definition of Done is the **"Documentation"** section of this checklist, adapted as follows:

- [ ] The document meets the minimum content expected by the corresponding section's template
- [ ] No blank spaces or unfilled placeholders remain
- [ ] It was reviewed by at least one other team member
- [ ] It is consistent with the other existing documents (no contradictions)
